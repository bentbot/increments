...
exports.setup = function (data, cb) {
    ...
    enableCookieProtection = data.cookies?data.cookies:false;
    enableInstanceKeyProtection = data.instance?data.instance:false;
    ...
    if (data.db.match(/(?:mongodb:\/\/)+/g)) {
        ...
        if ( mysqldb = mysql.createConnection(data.db) ) {
            return
        } else {
            if (cb) cb({ error: 500, message: 'Increment can not connect to MySQL! \n'+data.db + '\n' + err });
        }

    } else {
        if (cb) cb({ error: 500, message: 'Invalid database connection. Expecting url to match: mongodb://username:password@127.0.0.1:27017/collection or mysql://username:password@127.0.0.1:27017/db', db: data.db })
    }
}
...
exports.schema = function (poll, cb) {
    ...
    if (mysqldb) {
        ...
        VoterSchema = new mongo.Schema(defaultSchema);
...
exports.seed = function (cb) {
    ...
    mysql.query("CREATE TABLE voters (\
        id int(11) NOT NULL AUTO_INCREMENT,\
        poll varchar(100) NOT NULL,\
        name varchar(100) NOT NULL,\
        data longtext() NOT NULL,\
        unique varchar("+48+") NOT NULL,\
        time date() NOT NULL,\
        PRIMARY KEY (id)"
    );
...
exports.countVotes = function (poll, cb) {
    ...
    // Async finish...
        ...
        // Count total votes
        Voter[poll].count({ poll: poll }).exec(function (err, total) {
