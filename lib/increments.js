    ...
    if (leaders && poll) {
        ...
        if ( typeof leaders[0] == 'object' ) {
            ...
            async.each( leaders, function (leader) {
                ...
                candidates[poll].push({ name: leader.name, id: identity });
            });
            ...
            async.each( leaders, function (leaderName) {
                ...
                candidates[poll].push({ name: leaderName, id: identity });
            });
            ...
            if (err && err.code =='ER_TABLE_EXISTS_ERROR') {
                mysql.query("DESCRIBE "+poll, function(err, description) {
                    ...
                    async.each(description, function(columnName, callForward) {
                        ...
                        let columnNames = new Array('id', 'time');

                        async.each( candidates[poll], function(candidate) {
                            columnNames.push( candidate.id );
                        });
                        ...
                        for (var i = 0; i <= columns.length; i++) { 
                            for (var e = 0; e <= columnNames.length; e++) { 
                                if( columns[i] && columnNames[e] )                                 
                                    if ( columnNames[e] == columns[i] )
                                        columnNames.splice( e, 1 );
                            }
                        }
                        ...
                        if ( columnNames.length > -1 ){
                            ...
                            let rules = new Array();
                            async.each( columnNames, function(column) {
                            ...
                            if ( rules.length > 0 ) {
                                ...
                                mysql.query("ALTER TABLE " + poll + rules, function( err, results ) {
                                    console.log("ALTER TABLE " + poll + rules);
...
exports.statistics = function (polls, cb) {
    ...
    if (typeof polls == 'string') {
        ...
        this.countVotes(polls, function (err, statistics) {
...
exports.countVotes = function (poll, cb) {
    ...
    // Async finish...
        ...
        if ( engine == 'mysql' ) {
            // Count all votes from 
            mysql.query("SELECT COUNT(*) FROM "+poll, function (err, count) {
                total = count
            });
        } 
...
exports.checkKey = function (vote, callback) {
    ...
    mysql.query( "SELECT FROM "+vote.poll+" WHERE "+pollKeys.join(), function(err, docs) {
  ...
  if ( engine == 'mysql' ) {
        ...
        mysql.query("SELECT FROM "+poll+" WHERE browser_cookie = "+vote.unique)

        Voter[vote.poll].findOne({ unique: vote.cookie }, function (err, voter) {
            if (err) throw (err);
            if ( voter ) {
                callback(true, voter);
            } else {
        ...
        Voter[vote.poll].findOne({ browser_cookie: vote.cookie }, function (err, voter) {
            if (err) throw (err);
            if ( voter ) {
                callback(true, voter);
            } else {
